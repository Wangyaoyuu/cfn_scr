...
def main_function(rank, world_size):
    ...
    for batchtest in testloader:
        ...
        with pt.no_grad():
            ...
            for i, j, k in zip(index.tolist(), F.softmax(yy, dim=-1).tolist(), F.softmax(zz, dim=-1).tolist()):
                testlabel[i[0], i[1], i[2], i[3]], testdist[i[0], i[1], i[2], i[3]] = j, k

            testsampler.update(index, yy_argmax, zz_argmax)
